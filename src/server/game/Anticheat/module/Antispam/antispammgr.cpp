...
{
    std::lock_guard<std::mutex> guard(_mutex);
    _temporaryCache[session->GetAccount()] = std::make_pair(getMSTime(), session);
}
...
AntispamMgr* AntispamMgr::instance()
{
    static AntispamMgr instance;
    return &instance;
}
}
